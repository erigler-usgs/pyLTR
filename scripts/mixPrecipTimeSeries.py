    ...
    for i,time in enumerate(timeRange[index0:index1]):
        try:
            ...
            if doBBEaverage and i > averagesize:
               bhpNorth.append((sum(bhpNorth[-averagesize:-1])+hp.sum() * 1.6e-21)/averagesize)
            else:
            ...
            # KeV/cm^2s to mW/m^2 to GW
            if doBBEaverage and i > averagesize:
               bhpSouth.append((sum(bhpSouth[-averagesize:-1])+hp.sum() * 1.6e-21)/averagesize)
            else:
            ...
            if doCUSPaverage and i > averagesize:
               chpNorth.append((sum(chpNorth[-averagesize:-1])+hp.sum() * 1.6e-21)/averagesize)
            else:
            ...
            # KeV/cm^2s to mW/m^2 to GW
            if doCUSPaverage and i > averagesize:
               chpSouth.append((sum(chpSouth[-averagesize:-1])+hp.sum() * 1.6e-21)/averagesize)
            else:
...
if __name__ == '__main__':
    ...
    filename = os.path.join(path, 'mixPrecipTimeSeries.pkl')
    ...
    filename = os.path.join(path, 'mixPrecipTimeSeries.png')
    print(('Writing ' + filename))
    pyLTR.Graphics.TimeSeries.MultiPlotN([dataNorth, dataSouth], 
                                         'datetime', ['diffusemono', 'bbe', 'cusp'], 
                                         ['r','b'], ['North', 'South'])
