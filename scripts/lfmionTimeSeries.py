#!/usr/bin/env python
"""
pyLTR LFM Ion Time Series

Execute './lfmionTimeSeries.py --help' for more information.
"""
...
if __name__ == '__main__':
    ...
    (path, run, runExt, t0, t1) = parseArgs()
    print((path, run, runExt))

    (dataNorth, dataSouth) = extractQuantities(path, run, runExt, t0, t1)
    
    # --- Dump a  pickle!
    print('Serializing pyLTR.TimeSeries object of LFMION data using the Python Pickle package.')
    filename = os.path.join(path, 'lfmionTimeSeries.pkl')
    ...
    print('Creating summary plot of LFMION time series data.')
    filename = os.path.join(path, 'lfmionTimeSeries.png')
    print(('Writing ' + filename))
