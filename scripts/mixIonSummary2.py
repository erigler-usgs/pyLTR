...
def CreateFrames(path, runName, t0, t1, hemisphere, configFile):
    ...
    if (configFile == None): 
       ...
       flxOpts={'min':0.,'max':10.0,'format_str':'%.1e','colormap':'jet'}
    ...
    for i,time in enumerate(timeRange[index0:index1]):
        try:
           ...
           flux=data.read('Number flux '+hemiSelect+' [1/cm^2 s]',time)
           vals=data.read('Average energy '+hemiSelect+' [keV]',time)
           efx = flux*vals*10.**4*1.60218*(10**-16)*(10**3)
           flx={'data':vals,'name':r'E. Flux','units':r'$\mathrm{mW/m^2}$'}
