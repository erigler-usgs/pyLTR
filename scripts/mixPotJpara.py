...
def CreateFrames(path, runName, t0, t1, hemisphere, configFile):
    ...
    if (configFile == None): 
       ...
       optsObject = {'pot':potOpts,
                     ...
                     'energy':engOpts,
                     'flux':flxOpts}
       configFilename=os.path.join(dirname,'PotJpara.config')
    ...
    for i,time in enumerate(timeRange[index0:index1]):
        try:
           ...
           pyLTR.Graphics.PolarPlot.OverPlotDict(longitude,colatitude,fac,psi,
                  facOpts,potOpts,userAxes=ax)
           savefigName = os.path.join(path,'figs',hemisphere,'frame_PotJpara_%05d.png'%i)
...
if __name__ == '__main__':
    ...
    if (not southOnly):
        ...
        if hasMovieConverter:
            movieEncoder.encode( frameDir, 'frame_PotJpara_00', '%03d', 'png', 'mix_PotJpara_north.'+movieExtension )
            
    if (not northOnly):
        frameDir=CreateFrames(dirPath, runName, t0, t1,'south',configFile)
        if hasMovieConverter:
            movieEncoder.encode( frameDir, 'frame_PotJpara_00', '%03d', 'png', 'mix_PotJpara_south.'+movieExtension )
