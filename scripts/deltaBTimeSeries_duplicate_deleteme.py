...
def extractQuantities(path='./', run='', 
    ...
    # sort observatory list by phi, then theta, then rho
    ...
    for i,time in enumerate(timeRange[index0:index1]):
        ...
        try:
           ...
           try:
              ...
              # resort dB_* according to phi, then theta, then rho
              phi = dB_obs[0]['data']
              theta = dB_obs[1]['data']
              rho = dB_obs[2]['data']
              # note, axis=0 and kind='heapsort' are important here
              dBSortIdx = p.lexsort(p.vstack((phi,theta,rho)))
              for i in range(len(dB_obs)):
                 dB_obs[i]['data'] =  dB_obs[i]['data'][dBSortIdx]
                 dB_ion[i]['data'] =  dB_ion[i]['data'][dBSortIdx]
                 dB_fac[i]['data'] =  dB_fac[i]['data'][dBSortIdx]
                 dB_mag[i]['data'] =  dB_mag[i]['data'][dBSortIdx]
              ...
              rionN_min, rionN_max = pyLTR.Physics.SSECS._edgeGrid((phiN,thetaN))
              ...
              rionS_min, rionS_max = pyLTR.Physics.SSECS._edgeGrid((phiS,thetaS))
              rionS_min.append(p.zeros(phiS.shape)+6500e3)
              rionS_max.append(p.zeros(phiS.shape)+p.Inf)
              
              
              # next, generate SSECS for Northern ionospheric currents only
              (rvN_ion, 
               JvN_ion, 
               dvN_ion) = pyLTR.Physics.SSECS.ssecs_sphere([rionN_min[0],rionN_min[1],rionN_min[2]], 
                                                           [rionN_max[0],rionN_max[1],rionN_min[2]], 
                                                           (JphiN_dict['data']/1e6, 
                                                            JthetaN_dict['data']/1e6), 
                                                           10, False)
              
              # next, generate SSECS for Northern hemisphere inside LFM inner boundary
              (rvN_IBin, 
               JvN_IBin, 
               dvN_IBin) = pyLTR.Physics.SSECS.ssecs_sphere([rionN_min[0],rionN_min[1],rionN_min[2]], 
              ...
              """
              # generate SSECS for Northern Hemisphere outside LFM inner boundary
              # (this is serving as a proxy for magnetosphere currents for now)
              (rvN_mag, 
               JvN_mag, 
               dvN_mag) = pyLTR.Physics.SSECS.ssecs_sphere([rionN_min[0],rionN_min[1],2.5*rionN_min[2]], 
                                                          [rionN_max[0],rionN_max[1],rionN_min[2]],
                                                          (JphiN_dict['data']/1e6, 
                                                           JthetaN_dict['data']/1e6), 
                                                          10, False)
              ...
              # next, generate SSECS for Southern ionospheric currents only
              (rvS_ion, 
               JvS_ion, 
               dvS_ion) = pyLTR.Physics.SSECS.ssecs_sphere([rionS_min[0],rionS_min[1],rionS_min[2]], 
                                                          [rionS_max[0],rionS_max[1],rionS_min[2]], 
                                                          (JphiS_dict['data']/1e6, 
                                                           JthetaS_dict['data']/1e6), 
                                                          10, False)
              
              # next, generate SSECS for Southern hemisphere inside LFM inner boundary
              (rvS_IBin, 
               JvS_IBin, 
               dvS_IBin) = pyLTR.Physics.SSECS.ssecs_sphere([rionS_min[0],rionS_min[1],rionS_min[2]], 
              ...
              """
              # generate SSECS for Southern Hemisphere outside LFM inner boundary
              # (this is serving as a proxy for magnetosphere currents for now)
              (rvS_mag, 
               JvS_mag, 
               dvS_mag) = pyLTR.Physics.SSECS.ssecs_sphere([rionS_min[0],rionS_min[1],2.5*rionS_min[2]], 
                                                          [rionS_max[0],rionS_max[1],rionS_min[2]],
                                                          (JphiS_dict['data']/1e6, 
                                                           JthetaS_dict['data']/1e6), 
                                                          10, False)
              ...
              (dBphiS_ion, 
               ...
               dBrhoS_ion) = pyLTR.Physics.BS.bs_sphere(rvS_ion, 
                                                       ...
                                                       dvS_ion, 
                                                       (obs_phi,obs_theta,obs_rho))
              # deltaB for Southern SSECS currents inside IB
              (dBphiS_IBin, 
               dBthetaS_IBin, 
