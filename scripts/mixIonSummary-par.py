...
def CreateFrames(path, runName, t0, t1, hemisphere, configFile, ncpus):
    ...
    args = ((i,time,hemiSelect,hemisphere,data,potOpts,facOpts,pedOpts,halOpts,engOpts,flxOpts,longitude,colatitude,path) for i,time in enumerate(timeRange[index0:index1]) )
    #pl=Pool(processes=8)
...
def PlotStuff(i,time,hemiSelect,hemisphere,data,potOpts,facOpts,pedOpts,halOpts,engOpts,flxOpts,longitude,colatitude,path):
        #try:
           #first read the data
           ...
           vals=data.read('Average energy '+hemiSelect+' [keV]',time)
           eng={'data':vals,'name':r'Energy','units':r'keV'}
           vals=data.read('Number flux '+hemiSelect+' [1/cm^2 s]',time)
...
if __name__ == '__main__':
    ...
    if (not southOnly):
        ...
        if hasMovieConverter:
            movieEncoder.encode( frameDir, 'frame_summary_00', '%03d', 'png', 'mix_summary_north.'+movieExtension )
    if (not northOnly):
       frameDir=CreateFrames(dirPath, runName, t0, t1,'south',configFile,ncpus)
       if hasMovieConverter:
            movieEncoder.encode( frameDir, 'frame_summary_00', '%03d', 'png', 'mix_summary_south.'+movieExtension )
