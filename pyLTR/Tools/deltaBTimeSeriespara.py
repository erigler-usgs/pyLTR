...
def parseArgs():
    ...
    return (path, run, t0, t1, obsList, geoGrid, ignoreBinary, binaryType, options.ncpus, multiPlot,outDir)
...
def _dipoleMF(dBts, geoGrid=False):
   ...
   for i,ts in enumerate(datetimes):
      ...
      #print(("SM: ",phiSM,thetaSM,rhoSM,Bphi,Btheta,Brho))
      ...
      #print(("GEO: ", phiGEO,thetaGEO,rhoGEO,Bphi,Btheta,Brho))
...
def extractQuantities(path='./', run='', 
    ...
    # create vector of LFM time steps
    trLFM = dLFM.getTimeRange()
    
    # check that LFM timeRanges are the same as MIX timeRanges
    ...
    #       to a 1-minute tolerance; MIX time steps will be used for all output
    ...
    # Pre-compute fixed coordinates for northern hemisphere
    ...
    # Pre-compute fixed coordinates for southern hemisphere
    xS = xN
    yS = -yN
    ...
    # Pre-compute fixed coordinates for magnetosphere
    xM = dLFM.read('X_grid', trLFM[index0]) # this is in cm
    yM = dLFM.read('Y_grid', trLFM[index0]) # this is in cm
    zM = dLFM.read('Z_grid', trLFM[index0]) # this is in cm
    ...
    args = ((i,doy0,time,t0,
        geoGrid,obs_phi,obs_theta,obs_rho,
        ignoreBinary,path,outDirName,
        dMIX,dLFM,trLFM,
        xN_dict,yN_dict,xS_dict,yS_dict,colatNdict,colatSdict,hgridcc,
        xJM,yJM,zJM,dVM,index0,index1,binaryType) for i,time in enumerate(timeRange[index0:index1]) )
    # divide cpu_count by 2 due to hyperthreading
    ...
    with Pool(processes=ncpus) as pool:
        #results = pool.starmap(do_dB,args)
        (t_doy,
        obsGEO[0],obsGEO[1],obsGEO[2],
        obsSM[0],obsSM[1],obsSM[2],
        dBNorth_total,dBEast_total,dBDown_total,
        dBNorth_iono,dBEast_iono,dBDown_iono,
        dBNorth_fac,dBEast_fac,dBDown_fac,
        dBNorth_mag,dBEast_mag,dBDown_mag)=zip(*pool.starmap(do_dB,args))
    #print('SHAPE:  ',n.shape(results))
...
def do_dB(i,doy0,time,t0,
    geoGrid,obs_phi,obs_theta,obs_rho,
    ignoreBinary,path,outDirName,
    dMIX,dLFM,trLFM,
    xN_dict,yN_dict,xS_dict,yS_dict,colatNdict,colatSdict,hgridcc,
    xJM,yJM,zJM,dVM,index0,index1,binaryType):
    ...
    t_doy=(doy0 + (time-t0).total_seconds()/86400.)
    obsGEO = [[],[],[]] # list of 3 lists
    obsSM = [[],[],[]] # list of 3 lists
    ...
    #print(("obs SM: ",obs_phi,obs_theta,obs_rho))
    #print(("obs CAR: ",x,y,z))
    #print(("obs GEO: ",obs_phi_geo,obs_theta_geo,obs_rho_geo))
    ...
    try:
       ...
       # compute total, Pedersen, and Hall current vectors
       ...
       # finally, generate magnetospheric currents from LFM file
       ...
       JxM,JyM,JzM = pyLTR.Physics.LFMCurrent(hgridcc,BxM,ByM,BzM,rion=1) # ...should be A/m^2 given default input units
       ...
       phiM,thetaM,rhoM,JphiM,JthetaM,JrhoM=pyLTR.transform.CARtoSPH(xJM,yJM,zJM,JxM,JyM,JzM)
       (dBphi_mag, 
        dBtheta_mag, 
        dBrho_mag) = pyLTR.Physics.BS.bs_sphere((phiM,thetaM,rhoM), 
                                                (JphiM,JthetaM,JrhoM), 
                                                dVM, 
                                                (obs_phi,obs_theta,obs_rho))
       ...
       # combine n/s dBs into lists of dicts, then dump to binary file 
       toPickle = {}
       ...
       dB_ion = [{'data': (dBphiN_ion + dBphiS_ion)*1e9,'units':'nT' ,'name':r'$\Delta \marthrm{B}_{\phi}$'},
                 {'data': (dBthetaN_ion + dBthetaS_ion)*1e9,'units':'nT' ,'name':r'$\Delta \marthrm{B}_{\theta}$'},
                 {'data': (dBrhoN_ion + dBrhoS_ion)*1e9,'units':'nT' ,'name':r'$\Delta \marthrm{B}_{\rho}$'}]
    ...
    #
    dBNorth_total=(-dB_ion[1]['data'] + -dB_fac[1]['data'] + -dB_mag[1]['data'])
    dBNorth_iono=(-dB_ion[1]['data'])
    dBNorth_fac=(-dB_fac[1]['data'])
    dBNorth_mag=(-dB_mag[1]['data'])
    
    dBEast_total=(dB_ion[0]['data'] + dB_fac[0]['data'] + dB_mag[0]['data'])
    dBEast_iono=(dB_ion[0]['data'])
    dBEast_fac=(dB_fac[0]['data'])
    dBEast_mag=(dB_mag[0]['data'])
    
    dBDown_total=(-dB_ion[2]['data'] + -dB_fac[2]['data'] + -dB_mag[2]['data'])
    dBDown_iono=(-dB_ion[2]['data'])
    dBDown_fac=(-dB_fac[2]['data'])
    dBDown_mag=(-dB_mag[2]['data'])
    ...
    #
    obsGEO[0]=(obs_phi_geo)
    obsGEO[1]=(obs_theta_geo)
    obsGEO[2]=(obs_rho_geo)
    obsSM[0]=(obs_phi)
    obsSM[1]=(obs_theta)
    obsSM[2]=(obs_rho)
    ...
    return (t_doy,
           ...
           dBNorth_iono,dBEast_iono,dBDown_iono,
           dBNorth_fac,dBEast_fac,dBDown_fac,
           dBNorth_mag,dBEast_mag,dBDown_mag)


if __name__ == '__main__':

    (path, run, t0, t1, obs, geoGrid, ignoreBinary, binaryType, ncpus,multiPlot, outDir) = parseArgs()

    (dBObs) = extractQuantities(path, run, t0, t1, obs, geoGrid, ignoreBinary, binaryType, outDir,ncpus)
