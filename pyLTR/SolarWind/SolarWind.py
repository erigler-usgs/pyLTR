...
class SolarWind(object):
    ...
    def __init__(self):
        ...
        results in a standard way via the a pyLTR.TimeSeries object.
    ...
    def _getTiltAngle(self, dateTime):
        ...
        """
        (x,y,z) = pyLTR.transform.SMtoGSM(0,0,1, dateTime)
    ...
    def _gsm2sm(self, dateTime, x,y,z):
        ...
        """        
        return pyLTR.transform.GSMtoSM(x,y,z, dateTime)
    ...
    def bxFit(self):
        ...
        x[abs(x)<0.01] = x[abs(x)<0.01]*0.
        print((x,y))

        import pyLTR.math
        #import statsmodels.api as sm        

        linearFit = ols(y,x.T, y_varnm='bx_sm', x_varnm=['by_sm','bz_sm'])
        ##Obtain information about the fit via the summary:
        #linearFit.summary()
        coef = linearFit.b

        #linearFit = sm.OLS(y,x.T).fit()
        #coef = linearFit.params
